...
const COVER_IMAGE_PATH = "./assets/images/wahegurusimrancover.jpg.mp3";
...
function SimranPlayer() {
  ...
  const imageLocalUriAdjusted = assets?.[1].localUri?.replace(".bin", "");

  const [assetDebugText, setAssetDebugText] = useState("");
  ...
  async function prepare() {
    ...
    try {
      ...
      if ((await TrackPlayer.getState()).currentTrack == null) {
        ...
        setAssetDebugText(
          `assets[1].localUri:${assets[1].localUri}\nassets[1].uri:${assets[1].uri}\nassets[0].localUri:${assets[0].localUri}\nassets[0].uri:${assets[0].uri},`,
        );
        const playlistId = await PlayerQueue.createPlaylist(
          LOCK_SCREEN_METADATA_BASE.album,
          undefined,
          imageLocalUriAdjusted,
        );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.content}>
        <Image
          source={imageLocalUriAdjusted}
          style={styles.cover}
          contentFit="contain"
        />
        <Text style={{ color: "#FFFFFF" }}>{assetDebugText}</Text>
